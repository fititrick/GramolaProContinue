$(document).ready(function() {
	...
	$("#tabsPerfil").tabs();
	$('.startButtonHolder').hide();
	$('#LinksContainer').hide();
	//Se le añade esto aqui para que esconda que la lista es privada.
	$('#messagePrivate').hide();
	if (start == true) {
		//if(idListaCompartida!="http://localhost/GramolaProject/" && idListaCompartida!="http://localhost/GramolaProject/index.html" &&  idListaCompartida!="http://www.gramolapro.com/GramolaProject/" && idListaCompartida!="http://www.gramolapro.com/GramolaProject/index.html" && idListaCompartida!="http://www.gramolapro.com/" ){  //esto es lo que hago si me llega una lista compartida.
		...
		document.f2.campo1.value = "http://www.gramolapro.com/index.html?v=" + idListaCompartida;
		...
		$('#b_facebookShare').click(function() {
			...
			var url = "http://www.facebook.com/sharer.php?u=http://www.gramolapro.com/index.html?v=" + idListaCompartida + "&t=Compartiendo Lista:" + idListaCompartida;
			nuevaVentana = window.open(url, "segundaPag", "toolbar=yes,location=no,resizable=no,height=200");

		});
		$('#b_twitterShare').click(function() {
			var dir = "http://www.gramolapro.com/index.html?v=" + idListaCompartida;
		...
		$.ajax({
			...
			success : function(response) {
				if (response == true) {
					...
					window.location = "http://www.gramolapro.com/";
				} else {
		...
		$.ajax({
			...
			success : function(response) {
				if (response != -1) {
					...
					var url = "http://www.facebook.com/sharer.php?u=http://www.gramolapro.com/index.html?v=" + response + "&t=Compartiendo Lista:" + response;
		...
		$.ajax({
			...
			success : function(response) {
				if (response != -1) {
					...
					var url = "https://twitter.com/intent/tweet?text=Compartiendo la lista:" + response + "&url=http://www.gramolapro.com?v=" + response;
					nuevaVentana = window.open(url, "segundaPag", "toolbar=yes,location=no,resizable=no,height=500");
				} else {
	...
	function data(html) {
		...
		function cambiaOnClickListas() {
			...
			this.muestraLinks = function muestraLinks() {
				...
				document.f1.campo1.value = "http://www.gramolapro.com/index.html?v=" + this.name;
